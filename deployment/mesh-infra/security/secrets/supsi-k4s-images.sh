...
# Usage:
#
#   $ ./supsi-k4s-images.sh TokenYouGotFromSUPSI
#
...
GITLAB_USR="gitlab+deploy-kitt4sme-k4s-cloud"
GITLAB_TOKEN=$1

kubectl create secret docker-registry supsi-k4s-images \
        --docker-server="https://gitlab-core.supsi.ch:5050" \
        --docker-username="${GITLAB_USR}" \
        --docker-password="${GITLAB_TOKEN}" \
        -o yaml --dry-run='client' | \
    sed 's!^  creationT.*$!  namespace: default\n  annotations:\n    sealedsecrets.bitnami.com/managed: "true"!' | \
    kubeseal -o yaml -w supsi-k4s-images.yaml
